...
class DecoderBlock(nn.Module):
    ...
    def forward(self, prev_seg: torch.Tensor, slices: torch.Tensor, text_cond: torch.Tensor) -> torch.Tensor:
        """
        prev_seg: (B, L)
        slices:   (B, S, L)
        text_cond:(B, C)
        ...
        """
        B, S, L = slices.shape
        assert L == self.slice_len
        # Attention over raw waveforms via cross-correlation with previous segment
        # logits[b, s] = <prev_seg[b], slices[b, s]> / sqrt(L) + text_bias
        dot = torch.einsum('bl,bsl->bs', prev_seg, slices) / math.sqrt(L)
        bias = self.text_to_logit_bias(text_cond)  # (B,1)
        dot = dot + bias  # broadcast
        attn = torch.softmax(dot, dim=-1)  # (B,S)

        # Weighted mixture of raw slices (linear superposition)
        mixed = torch.einsum('bs,bsl->bl', attn, slices)  # (B,L)
        # Optional text-conditioned gain on mixture
        gain = torch.sigmoid(self.text_to_mix_gain(text_cond))  # (B,1)
        mixed = mixed * gain

        # Deformation via small 1D conv (linear, shared)
        x = mixed.unsqueeze(1)  # (B,1,L)
        x = self.defconv(x).squeeze(1)  # (B,L)

        # Residual with previous segment to stabilize autoregression
        x = x + prev_seg

        # Linear projection across time
...
class WaveformMixerModel(nn.Module):
    ...
    def forward_generate(self, slices: torch.Tensor, texts: List[str], total_steps: int) -> torch.Tensor:
        """Autoregressively generate total_steps segments (each of length L) conditioned on slices and text.
        slices: (B, S, L)
        returns: (B, total_steps*L)
        """
        device = slices.device
        B, S, L = slices.shape
        text_cond = self.text_enc(texts).to(device)
        seg = torch.zeros(B, L, device=device)
        outs = []
        for t in range(total_steps):
            x = seg
            for layer in self.layers:
                x = layer(x, slices, text_cond)
            seg = x
            outs.append(seg)
...
def train(cfg: TrainConfig):
    ...
    rng.shuffle(all_idx)
    split = int(round(cfg.train_split * n))
    train_idx = all_idx[:split]
    val_idx   = all_idx[split:]
    print(f"[split] total={n}, train={len(train_idx)}, val={len(val_idx)}, ratio={cfg.train_split:.2f}")
    ...
    for epoch in range(1, cfg.epochs+1):
        # ===== 每轮开始前：随机打乱训练集顺序并保证整集遍历 =====
        # 这里用 SubsetRandomSampler，每次 __iter__ 都会随机打乱一次
        ...
        # 验证集顺序固定，便于对齐/对比
        val_sampler = torch.utils.data.SequentialSampler(val_idx)
        val_loader  = torch.utils.data.DataLoader(
            ds, batch_size=1, sampler=val_sampler,
            num_workers=8, pin_memory=True, persistent_workers=True, prefetch_factor=4
        )
        ...
        for batch in pbar:
            slices_np, target_np, meta = batch
            slices = slices_np[0].to(cfg.device)
            target = target_np[0].to(cfg.device)
            text = [meta['text'][0]]
            T = int(meta['T'][0])

            slices = slices.unsqueeze(0)   # (1,S,L)
            target = target.unsqueeze(0)   # (1,T*L)

            pred = model.forward_generate(slices, text, total_steps=T)  # (1, T*L)
            loss = F.mse_loss(pred, target)

            (loss / cfg.batch_accum).backward()
            if (global_step + 1) % cfg.batch_accum == 0:
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                pred = model.forward_generate(slices, text, total_steps=T)
                loss = F.mse_loss(pred, target)
...
if __name__ == "__main__":
    ...
    p_train.add_argument('--batch_accum', type=int, default=4)
